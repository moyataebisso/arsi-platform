...
function parseLocations(raw: string | null): LocationEntry[] {
  if (!raw) return []
  try {
...
export default async function OurHomesPage() {
  ...
  const [locationsRaw, business] = await Promise.all([
    getSiteSetting('locations'),
    getBusinessProfile(),
  ])

  let locations = parseLocations(locationsRaw)
  if (locations.length === 0) {
    // Single-location fallback from existing business profile
    locations = [
      {
